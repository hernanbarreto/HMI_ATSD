...
//			console.log(nodes);
		...
		function parseTextNode( node ) {
				...
				loader.load( 'fonts/helvetiker_regular.typeface.json', function ( font ) {
					...
					var fontSize = parseFloat(node.style.fontSize)*0.8;
					...
					text.position.x = parseFloat(node.attributes.x.value);
					text.position.y = parseFloat(node.attributes.y.value);										
					text.position.z = 0;					
					text.scale.y *= -1;
															
					text.name = node.id;
					for (var k = 0; k < 4; k++){
						var str = String(node.nextSibling.parentElement.attributes[k].nodeValue);
						var nlayer = str.includes("layer");
						var ndisplay = str.includes("display")
						if (!nlayer && !ndisplay){ 
						   var pk = k;
						   break;
						}   
					}
					text.layer = node.nextSibling.parentElement.attributes[pk].nodeValue;
		...
		function parsePathNode( node, style ) {
			...
			path.layer = node.nextSibling.parentElement.attributes[pk].nodeValue;
